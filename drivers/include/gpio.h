/**
 * @file gpio.h
 * @brief GPIO driver interface for STM32F446
 */
...
#define GPIO_AF12 0xCU // FMC/SDIO/OTG_HS
#define GPIO_AF13 0xDU // DCMI
 ...
 * @mode: Desired GPIO mode (Use defines, example: GPIO_MODE_INPUT or
 * GPIO_MODE_OUTPUT).
 * @pupd: Pull-up/Pull-down configuration (Use defines, example: GPIO_PUPD_NONE
 * or GPIO_PUPD_UP).
 * @af: Alternate Function mapping (Use defines, example: GPIO_AF1 for Timers.
 * Ignored if mode is not ALT).
 * @return: A pointer to a new allocated struct gpio instance, or NULL on
 * failure.
 */
struct gpio *gpio_new(struct gpio_registers *port, uint8_t pin, uint32_t mode,
                      uint32_t pupd, uint8_t af);
